...
void MD5Calc::update(const void* inbuf, size_t len) {
	struct Context *ctx = (struct Context *) &ctx_;
...
void MD5Calc::final(MD5Digest* digest) {
	struct Context *ctx = (struct Context *) &ctx_;
	unsigned count;
	unsigned char *p;
	...
	*p++ = 0x80;
	...
	memcpy(digest->d, ctx->buf, 16);
	memset(ctx, 0, sizeof(*ctx)); // In case it's sensitive
}

string MD5Calc::final() {
	MD5Digest dig;
	final(&dig);
...
string MD5String(boost::string_ref str) {
	return MD5Sum(str.data(), str.size());
}
